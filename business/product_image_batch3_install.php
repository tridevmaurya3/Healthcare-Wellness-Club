...
function pib3_h(mixed $value): string
{
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ((string)($_POST['action'] ?? '') !== 'install_batch3') {
            throw new RuntimeException('Unknown Batch 3 action.');
        }
        ...
        foreach ($previewRows as $sku => $row) {
            ...
            try {
                ...
                if ($imageId > 0) {
                    $save = $pdo->prepare("UPDATE product_images SET alt_text=?,is_primary=1,sort_order=0,source_page_url=NULL,source_name='Authorized cleaned product asset batch 3',verification_status='verified',verified_at=NOW() WHERE id=? AND product_id=?");
                    $save->execute([$altText, $imageId, $productId]);
                } else {
                    $save = $pdo->prepare("INSERT INTO product_images(product_id,image_url,alt_text,is_primary,sort_order,source_page_url,source_name,verification_status,verified_at) VALUES(?,?,?,1,0,NULL,'Authorized cleaned product asset batch 3','verified',NOW())");
                    $save->execute([$productId, $imageUrl, $altText]);
                    $imageId = (int)$pdo->lastInsertId();
                }

                security_step17_audit($pdo, (int)$user['id'], 'product_image_batch3_installed', 'product_image', $imageId, [
                    'sku' => $sku,
        ...
        $success = "Batch 3 finished: {$updated} updated, {$skipped} skipped.";
    }
} catch (Throwable $e) {
    $error = $e->getMessage();
}
...
<title>Product Image Batch 3 - Healthcare Wellness Club</title>
...
<div class="os-kicker">PRODUCT IMAGE BATCH 3</div>
...
<?php if ($error): ?><div class="pi-alert bad"><strong>Installer error:</strong> <?=pib3_h($error)?></div><?php endif; ?>
<?php if ($success): ?><div class="pi-alert good"><strong><?=pib3_h($success)?></strong></div><?php endif; ?>
<?php if (!$error): ?>
<section class="pi-grid">
<?php foreach ($previewRows as $row): $product=$row['product']; ?>
<article class="pi-card">
<div class="pi-image"><img src="<?=pib3_h($row['url'])?>" alt="<?=pib3_h($product['product_name'] ?? $row['sku'])?>"></div>
<b><?=pib3_h($row['sku'])?> · <?=pib3_h($product['product_name'] ?? 'Product not found')?></b>
<small>Repository file: <?=pib3_h($row['file'])?></small>
...
<form method="post">
<input type="hidden" name="csrf" value="<?=pib3_h($csrf)?>">
<input type="hidden" name="action" value="install_batch3">
<button type="submit">Set Batch 3 as Primary Images →</button>
</form>
...
<?php if ($results): ?><div class="pi-result"><?php foreach($results as $result): ?><div class="<?=$result['ok']?'':'bad'?>"><strong><?=pib3_h($result['sku'])?>:</strong> <?=pib3_h($result['message'])?></div><?php endforeach; ?></div><?php endif; ?>
</section>
<?php endif; ?>
</div>
