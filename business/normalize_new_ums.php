...
function nuw_phone(?string $value): array
{
    ...
    if (strlen($digits) === 12 && str_starts_with($digits, '91')) {
        ...
        return ['key' => $key, 'canonical' => '+91' . $key, 'valid' => true];
    }
    if (strlen($digits) === 10) {
        return ['key' => $digits, 'canonical' => '+91' . $digits, 'valid' => true];
    }

    return ['key' => '', 'canonical' => '', 'valid' => false];
}
...
function nuw_status(?string $value): string
{
    $label = mb_strtolower(nuw_trim($value), 'UTF-8');
    if ($label === '') {
        return 'active';
    }
    if (str_contains($label, 'inactive')) {
        return 'inactive';
    }
    if (str_contains($label, 'active')) {
        return 'active';
    }
    return 'active';
}

function nuw_existing_members(PDO $pdo, int $organizationId): array
{
    $stmt = $pdo->prepare('SELECT id, full_name, mobile FROM members WHERE organization_id=? ORDER BY id');
    $stmt->execute([$organizationId]);

    $byPhone = [];
    $byName = [];
    foreach ($stmt->fetchAll() as $member) {
        $phone = nuw_phone((string)($member['mobile'] ?? ''));
        $nameKey = nuw_name_key((string)($member['full_name'] ?? ''));
        if ($phone['valid']) {
...
try {
    ...
    $currentMemberCount = (int)$pdo->prepare('SELECT COUNT(*) FROM members WHERE organization_id=?')->execute([$organizationId]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($alreadyMapped === NEW_UMS_EXPECTED_ROWS && $pendingRows === 0) {
            ...
            $existing = nuw_existing_members($pdo, $organizationId);
            $prepared = [];
            $sourcePhones = [];
            $sourceNames = [];
            ...
            foreach ($rawRows as $rawRow) {
                ...
                if (!$phone['valid']) {
                    throw new RuntimeException('Source row ' . (int)$rawRow['source_row'] . ' has an invalid mobile number.');
                }
                if (!$date['valid']) {
                    throw new RuntimeException('Source row ' . (int)$rawRow['source_row'] . ' has an invalid UMS date.');
                }

                if (isset($sourcePhones[$phone['key']])) {
                    throw new RuntimeException('Duplicate mobile detected inside New UMS source. Normalization is blocked.');
                }
                if (isset($sourceNames[$nameKey])) {
                    throw new RuntimeException('Duplicate exact member name detected inside New UMS source. Normalization is blocked.');
                }
                $sourcePhones[$phone['key']] = true;
                $sourceNames[$nameKey] = true;

                $action = 'create';
                $matchedMemberId = null;

                if (isset($existing['by_phone'][$phone['key']])) {
                    $candidates = $existing['by_phone'][$phone['key']];
                    if (count($candidates) !== 1 || nuw_name_key((string)$candidates[0]['full_name']) !== $nameKey) {
                        throw new RuntimeException('Existing member identity conflict at source row ' . (int)$rawRow['source_row'] . '.');
                    }
                    $action = 'match';
                    $matchedMemberId = (int)$candidates[0]['id'];
                } elseif (isset($existing['by_name'][$nameKey])) {
                    $candidates = $existing['by_name'][$nameKey];
                    if (count($candidates) !== 1) {
                        throw new RuntimeException('Existing member name is ambiguous at source row ' . (int)$rawRow['source_row'] . '.');
                    }
                    $candidatePhone = nuw_phone((string)($candidates[0]['mobile'] ?? ''));
                    if ($candidatePhone['valid'] && $candidatePhone['key'] !== $phone['key']) {
                        throw new RuntimeException('Existing exact member name has a different mobile at source row ' . (int)$rawRow['source_row'] . '.');
                    }
                    $action = 'match';
                    $matchedMemberId = (int)$candidates[0]['id'];
                }
            ...
            try {
                ...
                foreach ($prepared as $row) {
                    ...
                    $sourceBase = 'legacy-xlsx:new-ums:' . $row['external_id'];

                    if ($row['action'] === 'create') {
                        $memberNotes = 'Normalized from New UMS raw source. Sponsor/team/supervisor relationship linking is intentionally deferred.';
                    ...
                    $umsInsert->execute([
                        ...
                        $clubId,
                        $memberId,
                        $row['ums_type'] !== '' ? $row['ums_type'] : null,
                        $row['ums_date'],
                        nuw_status($row['active_flag']),
                ...
                $success = 'New UMS normalization completed successfully inside one transaction.';
...
$readyToWrite = $error === null && $pendingRows === NEW_UMS_EXPECTED_ROWS && $alreadyMapped === 0;
...
<main class="imp-shell">
  <section class="imp-hero">
    <div>
      ...
      <h1>Convert only the verified New UMS source into Members and UMS records.</h1>
      <p>The write is limited to the 78 reviewed New UMS rows. It re-validates identity, runs in one database transaction, creates source-key protected records, updates raw mapping traceability and records an audit event.</p>
    </div>
  ...
  <section class="imp-grid">
    ...
    <article class="imp-card nuw-write">
      ...
      <p>This is the first normalized database write. It will not normalize Volume Points, orders, renewals, income, royalty or any of the six derived report sheets.</p>
      ...
      <?php elseif ($readyToWrite): ?>
        <div class="nuw-warning"><strong>Write boundary:</strong> new Members are created only when identity is unambiguous. Existing exact identities are matched, not duplicated. UMS records retain the original raw source link. Sponsor/team/supervisor relationship IDs are deliberately deferred.</div>
        <form method="post" class="imp-drop">
          <label class="nuw-check">
            <input type="checkbox" name="confirm_new_ums_write" value="yes" required>
            <span><strong>I confirm normalization of the verified New UMS dataset.</strong><br><small>Only Members + UMS records for these 78 source rows will be written.</small></span>
          </label>
      ...
      <?php if ($result !== null): ?>
        <div class="imp-derived-list" style="margin-top:14px">
          ...
          <div class="imp-derived-item"><b>Existing members matched</b><span><?= number_format((int)$result['matched_members']) ?></span></div>
    ...
    <aside class="imp-card nuw-boundary">
      ...
      <p>Step 8J intentionally keeps the first normalized write small and reversible at the transaction boundary.</p>
      <div class="imp-plan-list">
        <div class="imp-plan-row"><div><b>members</b><span>Create or safe-match identity</span></div><em>WRITE</em></div>
        <div class="imp-plan-row"><div><b>ums_records</b><span>Create source-linked UMS record</span></div><em>WRITE</em></div>
    ...
    <div class="imp-footer-note"><strong>Traceability:</strong> each UMS record points back to its raw Excel source row. Newly created members also retain their original raw source record. Relationship linking for Sponsor, Team and Active Supervisor is postponed to a dedicated pass so names are never guessed into IDs.</div>
  </section>
</main>
</body>
</html>
