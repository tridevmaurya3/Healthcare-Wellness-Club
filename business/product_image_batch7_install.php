...
function pib7_h(mixed $value): string
{
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ((string)($_POST['action'] ?? '') !== 'install_batch7') {
            throw new RuntimeException('Unknown Batch 7 action.');
        }
        ...
        foreach ($previewRows as $sku => $row) {
            ...
            try {
                ...
                if ($imageId > 0) {
                    $save = $pdo->prepare("UPDATE product_images SET alt_text=?,is_primary=1,sort_order=0,source_page_url=NULL,source_name='Authorized cleaned product asset batch 7 final',verification_status='verified',verified_at=NOW() WHERE id=? AND product_id=?");
                    $save->execute([$altText, $imageId, $productId]);
                } else {
                    $save = $pdo->prepare("INSERT INTO product_images(product_id,image_url,alt_text,is_primary,sort_order,source_page_url,source_name,verification_status,verified_at) VALUES(?,?,?,1,0,NULL,'Authorized cleaned product asset batch 7 final','verified',NOW())");
                    $save->execute([$productId, $imageUrl, $altText]);
                    $imageId = (int)$pdo->lastInsertId();
                }

                security_step17_audit($pdo, (int)$user['id'], 'product_image_batch7_installed', 'product_image', $imageId, [
                    'sku' => $sku,
        ...
        $success = "Batch 7 finished: {$updated} updated, {$skipped} skipped.";
    }
} catch (Throwable $e) {
    $error = $e->getMessage();
}
...
<form method="post">
<input type="hidden" name="csrf" value="<?=pib7_h($csrf)?>">
<input type="hidden" name="action" value="install_batch7">
<button type="submit">Set Final Batch as Primary Images →</button>
</form>
<p class="pi-note">This final installer changes only verified primary product-image mappings. Product names, MRP, VP, member tiers, inventory, orders and accounting remain unchanged.</p>
<?php if ($results): ?><div class="pi-result"><?php foreach($results as $result): ?><div class="<?=$result['ok']?'':'bad'?>"><strong><?=pib7_h($result['sku'])?>:</strong> <?=pib7_h($result['message'])?></div><?php endforeach; ?></div><?php endif; ?>
</section>
<?php endif; ?>
</div>
