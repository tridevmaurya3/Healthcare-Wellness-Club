...
$batch = [
    '0006' => ['file' => '0006.webp', 'label' => 'Herbal Aloe Concentrate - Unflavored (Original) - 500 ml'],
    '0020' => ['file' => '0020.webp', 'label' => 'Herbalife Calcium Tablets - 60 tablets'],
    '0064' => ['file' => '0064.webp', 'label' => 'Ocular Defense'],
    '0077' => ['file' => '0077.webp', 'label' => 'Herbal Control - 90 tablets'],
    '0111' => ['file' => '0111.webp', 'label' => 'Cell-U-Loss Advanced - 90 tablets'],
    '025K' => ['file' => '025K.webp', 'label' => 'Simply Probiotic - 1 g x 30 sachets'],
    '031K' => ['file' => '031K.webp', 'label' => 'H24 Rebuild Strength - 50 g x 10 sachets'],
    '046K' => ['file' => '046K.webp', 'label' => 'HN-Skin Booster - Orange - 10 g x 30 sachets'],
    '051K' => ['file' => '051K.webp', 'label' => 'Beta Heart - Vanilla Flavour - 15 g x 15 sachets'],
    '0555' => ['file' => '0555.webp', 'label' => 'Joint Support - 90 tablets'],
];

function pib1_h(mixed $value): string
{
...
try {
    ...
    foreach ($batch as $sku => $asset) {
        ...
        $previewRows[$sku] = [
            ...
            'asset' => $asset,
            'product' => $product,
            'exists' => is_file(dirname(__DIR__) . '/img/product-images/' . $asset['file']),
            'url' => '../img/product-images/' . $asset['file'],
        ];
    }

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        security_step17_verify_csrf((string)($_POST['csrf'] ?? ''));
        if ((string)($_POST['action'] ?? '') !== 'install_batch1') {
        ...
        foreach ($previewRows as $sku => $row) {
            ...
            try {
                ...
                if ($imageId > 0) {
                    $save = $pdo->prepare(
                        "UPDATE product_images
                         SET alt_text=?, is_primary=1, sort_order=0, source_page_url=NULL,
                             source_name='Authorized cleaned product asset batch 1',
                    ...
                    $save = $pdo->prepare(
                        "INSERT INTO product_images
                         ...
                         VALUES(?,?,?,1,0,NULL,'Authorized cleaned product asset batch 1','verified',NOW())"
                    );
                ...
                security_step17_audit(
                    $pdo,
                    (int)$user['id'],
                    'product_image_batch1_installed',
                    'product_image',
                    $imageId,
        ...
        $success = "Batch 1 finished: {$updated} updated, {$skipped} skipped.";
    }
} catch (Throwable $e) {
    $error = $e->getMessage();
}
...
<title>Product Image Batch 1 - Healthcare Wellness Club</title>
...
<div class="os-kicker">PRODUCT IMAGE BATCH 1</div>
<h1>Review and activate the first 10 cleaned product images.</h1>
<p>These store-ready assets were prepared from the product screenshots supplied for this project. This Administrator-only installer maps each image to its exact SKU as the verified primary catalog image. Running it again is safe and does not create duplicate image records.</p>
<div class="os-status-row"><span class="os-chip good">ADMIN ONLY</span><span class="os-chip">10 SKU images</span><span class="os-chip">Verified primary mapping</span></div>
</section>

<?php if ($error): ?><div class="pi-alert bad"><strong>Installer error:</strong> <?= pib1_h($error) ?></div><?php endif; ?>
<?php if ($success): ?><div class="pi-alert good"><strong><?= pib1_h($success) ?></strong></div><?php endif; ?>

<?php if (!$error): ?>
<section class="pi-grid">
<?php foreach ($previewRows as $row): $product = $row['product']; ?>
<article class="pi-card">
<div class="pi-image"><img src="<?= pib1_h($row['url']) ?>" alt="<?= pib1_h($product['product_name'] ?? $row['asset']['label']) ?>"></div>
<b><?= pib1_h($row['sku']) ?> · <?= pib1_h($product['product_name'] ?? $row['asset']['label']) ?></b>
<small><?= pib1_h($row['asset']['label']) ?></small>
...
<form method="post">
<input type="hidden" name="csrf" value="<?= pib1_h($csrf) ?>">
<input type="hidden" name="action" value="install_batch1">
<button type="submit">Set Batch 1 as Primary Images →</button>
</form>
<p class="pi-note">This action changes only product-image mapping. Product names, prices, VP, tiers, inventory, orders and accounting are not modified.</p>
<?php if ($results): ?><div class="pi-result"><?php foreach ($results as $result): ?><div class="<?= $result['ok'] ? '' : 'bad' ?>"><strong><?= pib1_h($result['sku']) ?>:</strong> <?= pib1_h($result['message']) ?></div><?php endforeach; ?></div><?php endif; ?>
</section>
<?php endif; ?>
</div>
</body>
