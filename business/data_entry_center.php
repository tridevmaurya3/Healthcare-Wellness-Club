...
$sourceTotal=0; $sourceMapped=0; $sourcePending=0; $members=[]; $operators=[]; $coachLevels=[]; $recentManual=[];
...
try {
    ...
    if (business_table_exists($pdo,'system_users') && business_table_exists($pdo,'organization_user_access')) {
        $stmt=$pdo->prepare("SELECT u.id,u.full_name,u.email,a.role_code FROM system_users u JOIN organization_user_access a ON a.user_id=u.id AND a.organization_id=? WHERE u.is_active=1 AND a.is_active=1 AND a.role_code IN ('admin','coach') ORDER BY FIELD(a.role_code,'admin','coach'),u.full_name");
        $stmt->execute([$organizationId]); $operators=$stmt->fetchAll();
    }
    if (business_table_exists($pdo,'coach_level_labels')) {
        $stmt=$pdo->prepare("SELECT level_code,level_name FROM coach_level_labels WHERE organization_id=? AND status='active' ORDER BY sort_order,level_name");
        $stmt->execute([$organizationId]); $coachLevels=$stmt->fetchAll();
    }
...
function de_options(array $options,string $name,string $placeholder='Choose option'): string {
    $selected=(string)($_POST[$name]??'');$html='<option value="">'.de_h($placeholder).'</option>';
    foreach($options as $value=>$label)$html.='<option value="'.de_h($value).'"'.($selected===(string)$value?' selected':'').'>'.de_h($label).'</option>';
    return $html;
}
function de_operator_options(array $operators,string $name): string {
    $selected=(string)($_POST[$name]??'');$html='<option value="">Choose Administrator / Coach</option>';
    foreach($operators as $o){$value=(string)$o['full_name'];$label=$value.' • '.strtoupper((string)$o['role_code']);$html.='<option value="'.de_h($value).'"'.($selected===$value?' selected':'').'>'.de_h($label).'</option>';}
    return $html;
}
?>
<!doctype html>
...
<div class="de-field wide"><label>Member Name</label><input name="full_name" value="<?= de_old('full_name') ?>" required autocomplete="off" maxlength="180" placeholder="Full member name"></div><div class="de-field"><label>Mobile</label><input name="mobile" value="<?= de_old('mobile') ?>" inputmode="tel" autocomplete="off" placeholder="Optional • 10–15 digits"></div><div class="de-field"><label>UMS Date</label><input type="date" name="ums_date" value="<?= de_old('ums_date',$today) ?>" required></div><div class="de-field"><label>UMS Type</label><select name="ums_type"><?=de_options(['New UMS'=>'New UMS','First Set'=>'First Set','Second Set'=>'Second Set','Active UMS'=>'Active UMS'],'ums_type','Choose UMS type')?></select></div><div class="de-field"><label>Status</label><select name="status"><option value="active" <?= de_selected('status','active') ?>>Active</option><option value="inactive" <?= de_selected('status','inactive') ?>>Inactive</option></select></div><div class="de-field half"><label>Team / Managed By</label><select name="team"><?=de_operator_options($operators,'team')?></select><span class="de-help">Live Administrator and Coach accounts only.</span></div>
