...
function admin_global_forms_markup(bool $insideBusiness, string $role): string
{
    ...
    $html='<div class="hwc-form-hub" data-designer="'.$base.'form_designer.php" data-schema="'.$base.'dynamic_form_schema.php"><button class="hwc-form-fab" type="button" aria-expanded="false"><b>＋</b><span>Forms</span></button><section class="hwc-form-drawer" hidden><header><div><small>SMART FORM CENTER</small><strong>Data Entry &amp; Form Design</strong></div><button type="button" data-form-close aria-label="Close forms">×</button></header><div class="hwc-form-list">';
    foreach($forms as $key=>[$label,$url]){$html.='<article><strong>'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'</strong><div><button class="entry" type="button" data-entry-url="'.$base.htmlspecialchars($url,ENT_QUOTES,'UTF-8').'" data-entry-label="'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'">Entry</button><button type="button" data-form-key="'.htmlspecialchars($key,ENT_QUOTES,'UTF-8').'" data-form-label="'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'">Design</button></div></article>';}
...
(()=>{const hub=document.querySelector('.hwc-form-hub');if(!hub)return;const fab=hub.querySelector('.hwc-form-fab'),drawer=hub.querySelector('.hwc-form-drawer'),modal=hub.querySelector('.hwc-form-modal'),frame=modal.querySelector('iframe'),title=modal.querySelector('[data-form-modal-title]');const closeDrawer=()=>{drawer.hidden=true;fab.setAttribute('aria-expanded','false')};const closeModal=()=>{modal.hidden=true;frame.src='about:blank'};const openModal=(src,label)=>{title.textContent=label;frame.title=label;frame.src=src;modal.hidden=false;closeDrawer()};fab.addEventListener('click',()=>{drawer.hidden=!drawer.hidden;fab.setAttribute('aria-expanded',drawer.hidden?'false':'true')});hub.querySelector('[data-form-close]').addEventListener('click',closeDrawer);hub.querySelector('[data-form-modal-close]').addEventListener('click',closeModal);hub.querySelector('.hwc-form-modal-bg').addEventListener('click',closeModal);hub.querySelectorAll('[data-entry-url]').forEach(b=>b.addEventListener('click',()=>openModal(b.dataset.entryUrl,(b.dataset.entryLabel||'Form')+' • Data Entry')));hub.querySelectorAll('[data-form-key]').forEach(b=>b.addEventListener('click',()=>openModal(hub.dataset.designer+'?form='+encodeURIComponent(b.dataset.formKey),(b.dataset.formLabel||'Form')+' • Designer')));document.addEventListener('keydown',e=>{if(e.key==='Escape'){if(!modal.hidden)closeModal();else closeDrawer()}});
