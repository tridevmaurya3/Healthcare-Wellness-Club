...
function admin_global_forms_markup(bool $insideBusiness, string $role): string
{
    ...
    foreach($forms as $key=>[$label,$url]){$html.='<article><strong>'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'</strong><div><button class="entry" type="button" data-entry-key="'.htmlspecialchars($key,ENT_QUOTES,'UTF-8').'" data-entry-url="'.$base.htmlspecialchars($url,ENT_QUOTES,'UTF-8').'" data-entry-label="'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'">Entry</button><button type="button" data-form-key="'.htmlspecialchars($key,ENT_QUOTES,'UTF-8').'" data-form-label="'.htmlspecialchars($label,ENT_QUOTES,'UTF-8').'">Design</button></div></article>';}
