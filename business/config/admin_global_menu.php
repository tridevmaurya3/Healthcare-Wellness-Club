...
document.addEventListener('pointerdown',event=>{const hub=document.querySelector('.hwc-form-hub');if(!hub||hub.contains(event.target))return;const drawer=hub.querySelector('.hwc-form-drawer'),fab=hub.querySelector('.hwc-form-fab');if(drawer&&!drawer.hidden){drawer.hidden=true;fab?.setAttribute('aria-expanded','false')}});
