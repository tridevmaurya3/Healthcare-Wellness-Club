...
function product_step11_ensure(PDO $pdo): void
{
    ...
    if ($missing) {
        ...
        foreach (preg_split('/;\s*(?:\r?\n|$)/', $sql) ?: [] as $statement) {
            ...
            try { $pdo->exec($statement); }
                ...
                if (!str_contains($msg, 'duplicate column') && !str_contains($msg, 'already exists')) throw $e;
            }
        }
    }
    ...
    foreach (product_step11_support_tables() as $table) if (!business_table_exists($pdo,$table)) throw new RuntimeException('STEP 11 support table missing: '.$table);
}
...
function product_step11_seed_data(): array
{
    ...
    if (!is_array($data) || count($data['products'] ?? []) !== 64) throw new RuntimeException('Authoritative STEP 11 price seed must contain exactly 64 reviewed products/items.');
    return $data;
}
...
function product_step11_market(PDO $pdo, int $organizationId): array
{
    ...
    $stmt->execute([$organizationId]); $market=$stmt->fetch();
...
function product_step11_tiers(PDO $pdo, int $organizationId, int $marketId): array
{
    ...
    $tiers=[
      ...
      ['ASSOC_25','Associate 25%','associate',25,60],['ASSOC_35','Senior Consultant 35%','associate',35,70],
      ['ASSOC_42','Qualified Producer / Success Builder 42%','associate',42,80],['ASSOC_50','Supervisor 50%','associate',50,90],['PU_PRICE','PU Price','associate',0,100],
    ];
    ...
    $stmt->execute([$organizationId,$marketId]); $out=[]; foreach($stmt->fetchAll() as $r)$out[$r['tier_code']]=$r; return $out;
}

function product_step11_online_images(): array
{
    return [
      'formula1'=>['url'=>'https://www.jiomart.com/images/product/original/rvbdgktwwz/herbalife-nutrition-formula1-shake-mango-flavor-with-afresh-energy-drink-elaichi-ginger-flavor-product-images-orvbdgktwwz-p601050416-0-202305161207.jpg?im=Resize%3D%28420%2C420%29','page'=>'https://www.jiomart.com/p/groceries/herbalife-nutrition-formula1-shake-mango-flavor-with-afresh-energy-drink-elaichi-ginger-flavor/601050416','source'=>'JioMart - online packaging reference','status'=>'needs_review'],
      'afresh'=>['url'=>'https://www.jiomart.com/images/product/original/rv7mdh68jv/herbalife-nutrition-ocular-defence-for-eye-health-200-g-afresh-cinnamon-product-images-orv7mdh68jv-p601430548-0-202305121528.jpg?im=Resize%3D%281000%2C1000%29','page'=>'https://www.jiomart.com/p/groceries/herbalife-nutrition-ocular-defence-for-eye-health-200-g-afresh-cinnamon/601430548','source'=>'JioMart - online packaging reference','status'=>'needs_review'],
      'ocular'=>['url'=>'https://www.jiomart.com/images/product/original/rv7mdh68jv/herbalife-nutrition-ocular-defence-for-eye-health-200-g-afresh-cinnamon-product-images-orv7mdh68jv-p601430548-0-202305121528.jpg?im=Resize%3D%281000%2C1000%29','page'=>'https://www.jiomart.com/p/groceries/herbalife-nutrition-ocular-defence-for-eye-health-200-g-afresh-cinnamon/601430548','source'=>'JioMart - Ocular Defense packaging reference','status'=>'needs_review'],
      'niteworks'=>['url'=>'https://www.jiomart.com/images/product/original/rvoebvkuna/herbalife-lemon-niteworks-drink-300g-product-images-orvoebvkuna-p602136788-0-202306041200.jpg?im=Resize%3D%28420%2C420%29','page'=>'https://www.jiomart.com/p/groceries/herbalife-lemon-niteworks-drink-300g/602136788','source'=>'JioMart - Niteworks 300g packaging reference','status'=>'needs_review'],
    ];
}
...
{
    product_step11_ensure($pdo); $data=product_step11_seed_data(); $ctx=product_org_context($pdo); $orgId=(int)$ctx['organization_id'];
    $market=product_step11_market($pdo,$orgId); $marketId=(int)$market['id']; $tiers=product_step11_tiers($pdo,$orgId,$marketId);
    ...
    try{
      ...
      $imgExists=$pdo->prepare("SELECT COUNT(*) FROM product_images WHERE product_id=? AND is_primary=1");$count=0;$images=0;
      foreach($data['products'] as $p){
        $productCode='HL-IN-'.$p['stock'];$prodUp->execute([$orgId,$categoryIds[$p['category']]??null,$productCode,$p['stock'],$p['name'],$p['stock'],'Herbalife',$p['pack_size'],$p['pack_unit']]);$prodGet->execute([$orgId,$productCode]);$productId=(int)$prodGet->fetchColumn();
        ...
        $kind=(string)($p['kind']??'nutrition');$prices=[];
        if($kind==='nutrition'){
          ...
          $prices=['ASSOC_RETAIL'=>$a[2],'ASSOC_EARN_BASE'=>$a[3],'PU_PRICE'=>$p['pu_price']??null];
        }elseif($kind==='application'){
          $prices=['ASSOC_RETAIL'=>$a[2],'PU_PRICE'=>$p['pu_price']??0];
        }
...
function product_step11_catalog_rows(PDO $pdo, int $organizationId, string $q='', string $category='ALL'): array
{
    ...
    $stmt=$pdo->prepare($sql);$stmt->execute($args);$rows=$stmt->fetchAll();$tierStmt=$pdo->prepare("SELECT t.tier_code,tp.price_amount FROM product_tier_prices tp JOIN product_discount_tiers t ON t.id=tp.discount_tier_id WHERE tp.price_version_id=? AND t.status='active' ORDER BY t.sort_order");
    foreach($rows as &$r){$tierStmt->execute([(int)$r['price_version_id']]);$r['tiers']=[];foreach($tierStmt->fetchAll() as $t)$r['tiers'][$t['tier_code']]=(float)$t['price_amount'];}unset($r);return $rows;
}

function product_step11_price_for(array $row,string $tier): float
{
    if($tier==='MRP')return(float)$row['mrp'];return isset($row['tiers'][$tier])?(float)$row['tiers'][$tier]:(float)$row['mrp'];
}