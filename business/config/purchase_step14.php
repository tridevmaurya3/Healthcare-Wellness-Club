...
{
    $sql="SELECT s.*,COALESCE(x.billed,0) billed,COALESCE(x.return_total,0) return_credit,COALESCE(x.paid,0) paid,COALESCE(x.outstanding,0) outstanding,COALESCE(x.open_bills,0) open_bills FROM purchase_suppliers s LEFT JOIN (SELECT b.supplier_id,SUM(CASE WHEN b.status='active' THEN b.total_amount ELSE 0 END) billed,SUM(CASE WHEN b.status='active' THEN b.return_credit ELSE 0 END) return_total,SUM(CASE WHEN b.status='active' THEN b.paid_amount ELSE 0 END) paid,SUM(CASE WHEN b.status='active' THEN GREATEST(0,b.total_amount-b.return_credit-b.paid_amount) ELSE 0 END) outstanding,SUM(CASE WHEN b.status='active' AND b.payment_status<>'paid' THEN 1 ELSE 0 END) open_bills FROM purchase_bills b WHERE b.organization_id=? GROUP BY b.supplier_id) x ON x.supplier_id=s.id WHERE s.organization_id=?";
