...
function cm_cart_quote(PDO $pdo, int $orgId, array $cart, string $mode): array
{
    ...
    if(business_table_exists($pdo,'public_site_content')){
        $s=$pdo->prepare("SELECT content_key,content_value FROM public_site_content WHERE organization_id=? AND content_key IN ('combo_enabled','combo_discount_percent','combo_product_ids','combo_product_1','combo_product_2','combo_product_3')");$s->execute([$orgId]);$cfg=[];foreach($s->fetchAll() as $r)$cfg[(string)$r['content_key']]=(string)$r['content_value'];
        if(($cfg['combo_enabled']??'0')==='1'){$decoded=json_decode((string)($cfg['combo_product_ids']??''),true);$comboIds=is_array($decoded)?array_values(array_unique(array_filter(array_map('intval',$decoded)))):[];if(!$comboIds)$comboIds=array_values(array_unique(array_filter([(int)($cfg['combo_product_1']??0),(int)($cfg['combo_product_2']??0),(int)($cfg['combo_product_3']??0)])));$comboPercent=max(0,min(100,(float)($cfg['combo_discount_percent']??0)));$cartQty=[];foreach($rawItems as $raw)$cartQty[(int)$raw['product']['id']]=(int)$raw['qty'];$comboEligible=count($comboIds)>=2&&$comboPercent>0&&!array_filter($comboIds,static fn(int $id):bool=>($cartQty[$id]??0)<1);}
    }
