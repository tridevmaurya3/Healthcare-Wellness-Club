...
function bi_step22_metrics(PDO $pdo,int $orgId,string $start,string $end):array{
    ...
    if($m['new_leads']>0)$m['conversion_rate']=round($m['lead_conversions']/$m['new_leads']*100,2);
...
function bi_step22_targets(PDO $pdo,int $orgId,string $start,string $end,array $metrics):array{
    $s=$pdo->prepare("SELECT * FROM bi_targets WHERE organization_id=? AND status='active' AND period_start<=? AND period_end>=? ORDER BY metric_code,target_name");$s->execute([$orgId,$end,$start]);$cat=bi_step22_metric_catalog();$out=[];foreach($s->fetchAll() as $r){$code=(string)$r['metric_code'];$actual=array_key_exists($code,$metrics)?$metrics[$code]:null;$target=(float)$r['target_value'];$direction=(string)$r['direction_code'];$pct=null;$met=null;if($actual!==null&&abs($target)>0.000001){$pct=round((float)$actual/$target*100,1);$met=$direction==='lower_better'?(float)$actual<=$target:(float)$actual>=$target;}$out[]=$r+['metric_name'=>$cat[$code][0]??$code,'actual_value'=>$actual,'achievement_pct'=>$pct,'met'=>$met];}return$out;
}
